...
const openedViews = ref<string[]>(['event']) // 默认打开
...
<template>
  <div class="app-shell">
        ...
        <div class="tabs-bar">
          ...
          >
            {{ menuItems.find(m => m.id === tab)?.label
              || systemSubMenus.find(s => s.id === tab)?.label
              || tab }}
    ...
    <!-- 使用 Teleport 将子菜单渲染到 body，实现右侧浮动效果（类似 VSCode） -->
    <Teleport to="body">
